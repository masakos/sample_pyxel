...
JUMP_POWER     = -6   # ジャンプ力　　　　　（おすすめ範囲: -4〜-10）
GRAVITY        = 0.5  # 重力の強さ　　　　　（おすすめ範囲: 0.2〜1.0）
...
GROUND_Y         = 104  # 地面のY座標（プレイヤーが立つ位置）
...
class App:
    ...
    def update_play(self):
        ...
        if self.is_jumping:
            print(f"y={self.player_y:.2f}, vy={self.player_vy:.2f}")
    ...
    def draw(self):
        ...
        # blt(画面x, 画面y, 画像番号, 画像内x, 画像内y, 幅, 高さ, 透明色)
