...
-----
MOVE_SPEED (プレイヤーの速さ):
左右キー入力時にプレイヤーのX座標を ± この値だけ移動。値が大きいほど速く移動し、カメラスクロールも速くなる。範囲1〜5で調整推奨。

JUMP_POWER (ジャンプ力):
スペースキー入力時に縦速度（player_vy）をこの値（負数）に設定。上向きの初速が強くなり、高く跳べる。値が小さい（例: -10）ほど高く跳ぶ。

GRAVITY (重力の強さ):
毎フレームplayer_vy += GRAVITYで下向きに加速。値が大きいほど落下が速く、重い感じになる。ジャンプの高さも影響。範囲0.2〜1.0推奨。

ENEMY_SPEED (敵の移動速さ):
毎フレーム敵のX座標を - この値だけ移動。値が大きいほど敵が速く左に進み、回避しにくくなる。範囲1〜4推奨。

ENEMY_INTERVAL (敵の出やすさ):
random.randint(0, ENEMY_INTERVAL) == 0の確率で敵が出現。値が小さいほど出現頻度が高く、ゲームが難しくなる。範囲20〜120推奨。
